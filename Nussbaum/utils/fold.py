...
def calibrate(calpath, plots_on=True):
    ...
    calibration = np.genfromtxt(calpath) 
...
def fold(popt,datapath) :
    ...
    # NOTE, this is set up for spectra with 1023 channels.
    vLHS = np.arange(0.5, 511.5, 1)
    velocityLHS = vLHS*LHSm+LHSc
    countLHS=datatofit[0:511]
    
    #Once we know the coefficients of the linear equation we can convert channel to velocity for RHS
    vRHS = np.arange(512.5, 1023.5, 1) 
