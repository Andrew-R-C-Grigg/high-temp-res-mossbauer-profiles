...
The code to automatically download Mössbauer data from Wissoft software 
at a user defined time interval
and control a Lake Shore Model 335 controller
...
from lakeshore import Model335, Model335InputSensorSettings, InstrumentException
...
# check if it is already open
    app = Application('uia').connect(path=r"C:\Program Files (x86)\Wissel\Wissoft 2010\Wissoft")
    dlg=app.window(title_re="Device W-9-0820 Data from Device")

except:
# if it is not already open, open now
    app = Application('uia').start(r"C:\Program Files (x86)\Wissel\Wissoft 2010\Wissoft")
    dlg = app['Wissoft']
    app['dlg']['USB: W-9-0820'].click()
    dlg=app.window(title_re="Device W-9-0820 Data from Device")
    app['dlg']['AutoUpdate'].click()
...
def create_temp_plot_window():
    ...
    """
    plotting_script_path = r"C:\Users\Admin\Desktop\Nussbaum\matrix_plotting.py"
...
def clear():
    """a function to clear the memory of the Wissoft software"""
    app['dlg']['Stop'].click() 
    app['dlg']['Clear Memory'].click() 
    app.ClearMemory.Yes.click()
...
def draw_temp_plot(tempA, tempB):
    """
    A dedicated function that ONLY handles drawing the temperature plot.
...
def update_temp_plot():
    ...
    try:
        # NOTE: This assumes your Model335 connection logic is available
        my_model_335 = Model335(57600)
        tempA = my_model_335.get_kelvin_reading("A")
        tempB = my_model_335.get_kelvin_reading("B")
        my_model_335.disconnect_usb()
        del my_model_335
        current_time = time.time()
...
def ramp_temperature(final_setpoint, loop_count, ns, then):
    ...
    # Establish a connection with the Lake Shore Model 335 controller.
    # This will raise an error if the connection fails.
    my_model_335 = Model335(57600)
    above_trigger=True

    def _start_ramp_logic(my_model_335):
        ...
        try:
            current_range = my_model_335.get_heater_range(1)
        except InstrumentException:
            current_range = 0  # HeaterRange.OFF is 0

        # If the heater is on, configure the output modes for both heaters.
        if current_range != 0:
            my_model_335.set_heater_output_mode(1, 1, "A") # Control Loop 1 -> Sensor A
            my_model_335.set_heater_output_mode(2, 1, "B") # Control Loop 2 -> Sensor B

        
        # Get the initial temperature to calculate the ramp parameters.
        start_temp = my_model_335.get_kelvin_reading("B")
        ...
        def _ramp_step(current_step, my_model_335):
            ...
            if not above_trigger:
                ...
                my_model_335.set_control_setpoint(1, final_setpoint[0])
                my_model_335.set_control_setpoint(2, final_setpoint[1]+offset)                
                my_model_335.disconnect_usb()
                del my_model_335
            ...
            my_model_335.set_control_setpoint(1, next_setpoint[0])
            my_model_335.set_control_setpoint(2, next_setpoint[1])
            tempA = my_model_335.get_kelvin_reading("A")
            tempB = my_model_335.get_kelvin_reading("B")
            draw_temp_plot(tempA, tempB)
            ...
            root.after(10000, lambda: _ramp_step(current_step + 1, my_model_335))

        # Start the ramp process at the first step.
        _ramp_step(1, my_model_335)

    # Schedule the main logic to start after a 200ms non-blocking pause.
    root.after(200, lambda: _start_ramp_logic(my_model_335))
...
def wait_for_temp_drop(setpoint, loop_count, ns, then):
    ...
    my_model_335 = None  # Initialize to None to ensure it exists for the 'finally' block.
    try:
        # Connect and Use
        ...
        tempA = my_model_335.get_kelvin_reading("A")
        tempB = my_model_335.get_kelvin_reading("B")
        my_model_335.set_control_setpoint(1, setpoint[0])
        my_model_335.set_control_setpoint(2, setpoint[1])
    except Exception as e:
...
def download_and_save(loop_count,ns):
    ...
    try:
        download_button=app['dlg']['Download']
        download_button.wait('visible enabled', timeout=30)
        download_button.click()
        
    except pywinauto.timings.TimeoutError:
        # Diagnose the screen if the button isn't found 
        # This code only runs if the .wait() method fails after 30 seconds.
        print("\nERROR: The 'Download' button was not found in time!")
        print("This was a random occurrence. Printing the current screen layout for debugging.")
        print("Look for unexpected pop-ups or dialogs in the list below.")
    ...
    try:
        save_as_button = app['dlg']['Save as...']
        save_as_button.wait('visible enabled', timeout=30)
        save_as_button.click()
        sub = app.dlg.window(title_re="Save As", class_name="#32770")
        sub.wait('ready', timeout=10)
    except pywinauto.timings.TimeoutError:
        label6_var.set('spectrum saving function timeout. Please press start to continue program')
        label6.update_idletasks()
    
    sub.FileNameCombo.type_keys(r'C:\Users\Admin\Documents\Mossbauer_auto_backup\log\{}{}{}_{}h{}m{}s__{:.0f}__{:.0f}.ws5'.format(t[0], t[1], t[2], t[3], t[4], t[5], ns, nsdiff))
    sub.Save.click()
    
    file_path = r'C:\Users\Admin\Documents\Mossbauer_auto_backup\log\{}{}{}_{}h{}m{}s__{:.0f}__{:.0f}.ws5'.format(t[0], t[1], t[2], t[3], t[4], t[5], ns, nsdiff)
    wait_until(5, 0.1, lambda: os.path.exists(file_path))
    
    loops_to_save = round(np.divide(600, float(e1.get())), 0)
    
    with open(r'C:\Users\Admin\Documents\Mossbauer_auto_backup\log\{}{}{}_{}h{}m{}s__{:.0f}__{:.0f}.ws5'.format(t[0], t[1], t[2], t[3], t[4], t[5], ns, nsdiff), mode='r') as csvfile:
        dreader = csv.reader(csvfile, delimiter='\t', quotechar='|')
        if loops_to_save == loop_count:
            loop_count = 1
            with open(r'C:\Users\Admin\Documents\Mossbauer_auto_backup\Mossbauer_backup_spectra\{}_{}_{}___{}h{}m{}s.dat'.format(t[0], t[1], t[2], t[3], t[4], t[5]), 'w', newline='') as f1:
                writer1 = csv.writer(f1)
                for n, row in enumerate(dreader):
                    if isfloat(row[0]) == True:
...
def measurement_loop(loop_count, ns, then):
    ...
    if abs(tempB - setpoint[1]) > 0.05:
        ...
        def _send_commands_and_reschedule(my_model_335):
            try:
                current_range = my_model_335.get_heater_range(1)
            except InstrumentException:
                current_range = 0
            if current_range != 0:
                ...
                my_model_335.set_heater_range(1, 3)
                my_model_335.set_heater_range(2, 3)
                my_model_335.set_control_setpoint(1, setpoint[0])
                my_model_335.set_control_setpoint(2, setpoint[1])
            # else:
        ...
        def _connect_and_correct():
            my_model_335 = Model335(57600)
            root.after(200, lambda: _send_commands_and_reschedule(my_model_335))
    ...
    try:
        ...
        if var3.get() == 1 and len(data1) == 1023:
            time_step = now - then
        ...
        if error_count == 4:
            app['dlg']['Stop'].click()
            root.after(4000, lambda: app['dlg']['Start'].click())
...
def start_measurement():
    ...
    shutil.rmtree(r'C:\Users\Admin\Documents\Mossbauer_auto_backup\log', ignore_errors=True)
    os.makedirs(r'C:\Users\Admin\Documents\Mossbauer_auto_backup\log', exist_ok=True)
    os.makedirs(r'C:\Users\Admin\Documents\Mossbauer_auto_backup\Mossbauer_backup_spectra', exist_ok=True)
